...
class WikiDpr(nlp.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            nlp.SplitGenerator(name=nlp.Split.TRAIN, gen_kwargs=downloaded_files, resources=downloaded_index),
        ]
            ...
            if os.path.exists(index_file):
                ...
                try:
                    downloaded_resources = dl_manager.download_and_extract(
                        {"embeddings_index": os.path.join(_INDEX_URL, self.config.index_file.format(split=dataset.split))}
                    )
                    dataset.load_faiss_index("embeddings", downloaded_resources["embeddings_index"])
                    print(">>> D")
